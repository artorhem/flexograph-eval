...
def do_bfs(blaze_index_file, blaze_adj_file, dataset):
    ...
    with open (outfile_csv, "w") as f:
        f.write("read_time(ms),algo_time(ms),mem_used(MB),start_node,num_threads\n")
    for start_node in random_starts:
        ...
        with open(outfile_log, "a") as flog, open(outfile_csv, "a") as fcsv:
            ...
            for i in range(REPEATS):
                ...
                fcsv.write(f"{read_time},{algo_time},{mem},{start_node},{NUM_WORKERS}\n")
...
def do_pagerank(blaze_index_file, blaze_adj_file, dataset):
    ...
    with open(outfile_log, "a") as flog, open(outfile_csv, "w") as fcsv:
        for i in range(REPEATS):
            ...
            fcsv.write(f"{read_time},{algo_time},{mem},{NUM_WORKERS}\n")
...
def main():
    ...
    for dataset in datasets:
        galois_file = f"{DATASET_DIR}/galois/{dataset}.gr"
        blaze_index_file = f"{TEMP_DIR}/{dataset}.gr.index"
        blaze_adj_file = f"{TEMP_DIR}/{dataset}.gr.adj.1.0" # <dataset>.adj.<num_disks>.<partition_id>
