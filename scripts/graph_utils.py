...
'''
This function is meant to be used with the .v file generated by graphalytics to generate random start nodes for BFS traversal
Open the file, read all the vertices, and select 20 random vertices to be used as starting points for BFS traversal.
The .v file only contains vertices that have some outgoing edge.
'''
def make_bfs_starts(infile, outfile):
	with open(infile, 'r') as inf:
		vertices = []
		for line in inf:
			vertices += [int(line.split()[0])]
		print("Total vertices in file", len(vertices))
		random_starts = random.sample(vertices, 20)
		with open(outfile, 'w') as outf:
...
if __name__ == '__main__':
	if len(sys.argv) < 3:
		print("Usage: python graph_utils.py [info|degree|zero|edgedeg|maxver|bfsver|dup_edges|make_bfs_starts] <filename> <optional: bfsver output file>")
		sys.exit(1)
	option = sys.argv[1]
	if option == 'info':
	...
	elif option == 'make_bfs_starts':
		make_bfs_starts(sys.argv[2], sys.argv[3])
	else:
		print("Usage: python graph_utils.py [info|degree|zero|edgedeg|maxver|bfsver|dup_edges|make_bfs_starts] <filename> <optional: bfsver output file>")
		sys.exit(1)
